...
=head3 chacha20_ietf_nonce

    my $nonce_ietf = $crypto_stream->chacha20_ietf_nonce();

Helper method to generate an extended random nonce to be used by C<$crypto_stream>.
...
C<null> bytes.
    ...
    my $counter = 121;
    my $nonce_ietf = $crypto_stream->chacha20_ietf_nonce($counter);
    $nonce_ietf =~ /^121\0+$/ or die;

B<NOTE:> nonce does not have to be random nor confidential, but it must never
be reused with the same key.

Bacause the nonce is 96 bits long, in order to prevent nonce reuse,
if a key is being reused, it is recommended to increment the
previous nonce instead of generating a random nonce every time
a new stream is required.
...
=head3 chacha20_ietf_bytes

    # stream of random bytes
    my $bytes = $crypto_stream->chacha20_ietf_bytes($num_of_bytes, $nonce_ietf, $key);

Generates unpredictable sequence of C<$num_of_bytes> bytes using given
C<$nonce_ietf> and C<$key>.

The length of the C<$bytes> equals the value of C<$num_of_bytes>.

Returns L<Data::BytesLocker> object.

=head3 chacha20_ietf_xor
...
Encrypts/decrypts the message using given C<$nonce_ietf> and C<$key>.
...
C<chacha20_ietf_xor_ic> is similar to L</chacha20_ietf_xor> function and
encrypts/decrypts the message using given C<$nonce_ietf> and C<$key>, but adds the
ability to set the initial value of the block counter to a non-zero value, C<$ic>.
