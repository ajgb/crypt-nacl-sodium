...
Encrypts the plaintext message using given C<$nonce> and pre-calculated
...
Verify and decrypt the secret message using given C<$nonce> and pre-calculated
C<$shared_key> (as returned by L</beforenm>).
...
Verify and decrypt the secret message C<$ciphertext> authenticated with C<$mac>
using given C<$nonce> and pre-calculated C<$shared_key> (as returned by
L</beforenm>).
