...
Encrypts the plaintext message using given C<$nonce> and precalculated
C<$shared_key> (as returned by L</beforenm>).
...
Verify and decrypt the secret message using given C<$nonce> and precalculated
C<$shared_key> (as returned by L</beforenm>).
...
=head2 decrypt_detached_afternm
...
Verify and decrypt the secret message C<$ciphertext> authenticated with C<$mac>
using given C<$nonce> and precalculated C<$shared_key> (as returned by
L</beforenm>).
...
Returns the length of precalculated shared key.
